...
class Database {
    ...
    def recreate() = {
        ...
        trajets.insertMany(newTrajets).join()
        ...
        gravites.insertMany(newGravites).join()
        ...
        categoriesDeManoeuvres.insertMany(newCategoriesDeManoeuvre).join()
        ...
        manoeuvres.insertMany(newManoeuvres).join()
        ...
        obstaclesFixes.insertMany(newObstaclesFixes).join()
        ...
        obstaclesMobiles.insertMany(newObstaclesMobiles).join()
        
        println("recreate done")
    }
}
